...
由于生成的图片和网页中的区域一致，类似截图，导致在不同设备上出来的图片各不相同

而且，如果想在生成的图片中做些调整，也无法实现，只能隐去指定节点
...
设想一下场景，一个页面，内容是某份报告，包含若干文字、表格、图表，有一个导出功能，生成当前报告的pdf版本

<!-- 示意图 -->

直接整屏截取肯定是不太好的，因为生成的样式不好看，文字也不能选中

可以只截取图表，把图表部分的截图和文字发给后端，在后端拼成pdf

那问题就简化成了：在一个页面截取多个区域的图片，且在各个设备生成的截图一致
